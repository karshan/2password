...
import           Crypto.Cipher.Types (KeySizeSpecifier(..), cipherKeySize, blockSize, makeIV, cbcEncrypt, cbcDecrypt, cipherInit)
import           Crypto.Error (CryptoFailable, eitherCryptoError)
import           Crypto.Hash (Digest)
import           Crypto.Hash.Algorithms (SHA256)
import           Crypto.Hash.IO (hashDigestSize, hashBlockSize)
import qualified Crypto.KDF.Scrypt as Scrypt (generate, Parameters(..))
import           Crypto.MAC.HMAC (HMAC, hmacGetDigest, hmac)
import           Crypto.Random (getRandomBytes)
import qualified Data.ByteArray as BA (unpack)
import           Data.ByteString (ByteString)
import qualified Data.ByteString as BS (pack, drop, take, length, splitAt, replicate)
...
scryptParams :: Int -- ^ Output Size
             -> Scrypt.Parameters
scryptParams = Scrypt.Parameters (2 ^ 14) 8 1
